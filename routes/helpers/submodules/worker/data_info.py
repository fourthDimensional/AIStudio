...
def generate_profile_report(dataset_key: str, apikey: str, redis_connection_info: dict) -> None:
    """
    Generate a profile report for a dataset and save it to Redis.

    :param redis_connection_info:
    :param apikey: Owner of the dataset
    :param dataset_key: The key of the dataset to generate a profile report for.
    """
    ...
    redis_client.json().set(f"profile_report:{apikey}:{dataset_key}", '$', json.loads(json_data)) # TODO ADD CONSISTENT STRING CLEANING

    return f"profile_report:{apikey}:{dataset_key}"
